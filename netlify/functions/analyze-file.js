...
// summary extracted synchronously in upload-file.js. Uses Groq's vision
// model (separate from the text chat default) since gpt-oss-120b has no
// image support; llama-4-scout does.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (file.kind !== "image") {
      // Non-images already have their summary from upload time — just return it.
    ...
    const { data: signedUrlData, error: signError } = await supabaseAdmin.storage
      .from("uploads")
      .createSignedUrl(file.storage_path, 300); // 5 minutes — just long enough for Groq to fetch it
